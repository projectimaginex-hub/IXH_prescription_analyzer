...
class Patient(models.Model):
    ...
    previous_medication = models.TextField(blank=True, null=True)
    date_created = models.DateTimeField(auto_now_add=True)

    def save(self, *args, **kwargs):
        # Clean the new text if provided
        if self.previous_medication:
            clean_text = re.sub(
                r'<[^>]*>', '', self.previous_medication).strip()

            # If patient already exists in DB → append new medication info
            if self.pk:  # pk exists means not a new record
                try:
                    old_record = Patient.objects.get(pk=self.pk)
                    if old_record.previous_medication:
                        # Append old + new text, separated by a newline
                        clean_text = f"{
                            old_record.previous_medication}\n{clean_text}"
                except Patient.DoesNotExist:
                    pass  # in case somehow not found

            self.previous_medication = clean_text
...
class Doctor(models.Model):
    ...
    about = models.TextField(
        blank=True, help_text="A short bio for the profile page.")
    professional_details = models.TextField(
        blank=True, help_text="Degrees, certifications, etc.")
...
class Prescription(models.Model):
    ...
    audio_recording = models.FileField(
        upload_to='audio_recordings/', blank=True, null=True)
...
# ... (rest of your models: Patient, Medicine, Symptom, etc.) ...
