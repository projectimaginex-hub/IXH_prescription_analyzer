...
from .models import Patient, Prescription, Doctor, Symptom, Medicine, Audio, LLMAudit, ContactSubmission
...
from django.shortcuts import render, redirect, get_object_or_404 # <-- ADD THI
from django.contrib import messages
from django.contrib.auth.decorators import login_required
from .llm_utils import analyze_transcription_text # We will define this helper function
from .analysis_pipeline import run_llm_analysis

# Should become:
from .llm_utils import analyze_transcription_text
from .analysis_pipeline import run_llm_analysis
...
def get_transcription_from_service(audio_file):
    # This is a placeholder for your actual transcription logic.
    # For now, it will return a dummy response.
    # In a real app, this would use the ASSEMBLYAI_API_KEY from settings.py
    return "Patient said, 'I have a severe headache, a fever of 101, and persistent body ache for the last two days. I am 35 years old and allergic to penicillin.' Doctor advised rest and a course of pain medication."
...
# --- NEW API VIEWS FOR LLM & AUDIO INTEGRATION ---

@csrf_exempt
@require_http_methods(["POST"])
def transcribe_audio_view(request):
    """
    Handles audio upload, saves the file, and returns transcription.
    """
    ...
    # 2. Get transcription (using placeholder/service)
    transcribed_text = get_transcription_from_service(audio_instance.audio_file.path)
...
def analyze_prescription_view(request):
    """
    Takes transcription text and runs the LLM analysis pipeline to suggest symptoms and medicines.
    """
    try:
        data = json.loads(request.body)
        transcribed_text = data.get('text', '')
        
        # We need a temporary Prescription object to run the analysis
        # For simplicity, we'll use a dummy patient/doctor for LLM suggestion, 
        # but a better approach is to create a 'draft' prescription object.
        
        # --- Dummy Analysis Call (For Symptoms & Medicine Suggestions) ---
        # The llm_utils.py needs a function to run the symptom/medicine prediction 
        # *without* immediately saving it to the Prescription model.
        # Assuming you've created this intermediary function:
        # from .llm_utils import run_preliminary_analysis
        
        # For now, we will use a dummy function and return dummy data
        
        # --- SIMULATE LLM OUTPUT ---
        suggested_symptoms = [
            {'name': 'Headache', 'confidence': 0.95},
            {'name': 'Fever', 'confidence': 0.88},
            {'name': 'Body Ache', 'confidence': 0.75},
        ]
        suggested_medicines = [
            {'name': 'Paracetamol', 'reason': 'To reduce fever and pain.', 'confidence': 0.92},
            {'name': 'Restol', 'reason': 'General pain relief.', 'confidence': 0.85},
        ]
...
def save_suggestion_view(request):
    """
    Saves a symptom or a medicine to the database from the user's selection.
    This is called when the user clicks 'Add' on a suggestion.
    """
    try:
        data = json.loads(request.body)
        item_name = data.get('name')
        item_type = data.get('type') # 'symptom' or 'medicine'
        
        if item_type == 'symptom':
            # Get or create the symptom
            Symptom.objects.get_or_create(name=item_name.capitalize())
            message = f"Symptom '{item_name}' saved to master list."
        elif item_type == 'medicine':
            # Get or create the medicine
...
from django.conf import settings
from django.views.decorators.csrf import csrf_exempt
from django.views.decorators.http import require_http_methods # <-- ENSURE THIS IS HERE
from reportlab.pdfgen import canvas
from reportlab.lib.pagesizes import letter
from reportlab.lib.units import inch
from django.contrib.auth import login, authenticate, logout
...
from .forms import UserForm, DoctorForm, DoctorProfileUpdateForm, ContactForm
from .analysis_pipeline import run_llm_analysis

# NOTE: analyze_transcription_text is currently commented out in the actual file you sent 
# from .llm_utils import analyze_transcription_text 
...
# --- Helper Function (Currently a Placeholder) ---
def get_transcription_from_service(audio_file):
    # This is a placeholder that will be replaced by your AssemblyAI logic for production
    return "Patient said, 'I have a severe headache, a fever of 101, and persistent body ache for the last two days. I am 35 years old and allergic to penicillin.' Doctor advised rest and a course of pain medication."
...
def prescription(request):
    if request.method == 'POST':
        # --- 1. GATHER ALL DATA ---
        audio_file = request.FILES.get('audio_recording') # Use the name from the JS or form
        transcribed_text = request.POST.get('transcriptionText', '')
        ...
        age_val = int(request.POST.get('age')) if request.POST.get('age') and request.POST.get('age').isdigit() else None
        weight_val = float(request.POST.get('weight')) if request.POST.get('weight') and request.POST.get('weight').replace('.', '', 1).isdigit() else None
        ...
        for s_name in [s.strip() for s in symptoms_list_str.split(',') if s.strip()]:
            symptom_obj, _ = Symptom.objects.get_or_create(name=s_name.capitalize())
            new_prescription.symptoms.add(symptom_obj)
            
        for m_name in [m.strip() for m in medicines_list_str.split(',') if m.strip()]:
            medicine_obj, _ = Medicine.objects.get_or_create(name=m_name.capitalize())
...
def analyze_prescription_view(request):
    try:
        ...
        analysis_results = analyze_transcription_text(transcribed_text, patient_info=patient_info) 
